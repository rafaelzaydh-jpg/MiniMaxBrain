...
// Process-global MoE expert provider used by the initial MiniMaxBrain
// integration. Registration happens before graph execution and is not mutated
// while worker threads are running.
static struct ggml_moe_expert_provider g_ggml_moe_expert_provider = { 0 };

int ggml_cpu_set_moe_expert_provider(const struct ggml_moe_expert_provider * provider) {
    if (provider == NULL) {
        memset(&g_ggml_moe_expert_provider, 0, sizeof(g_ggml_moe_expert_provider));
        return 0;
    }

    if (provider->begin == NULL || provider->get == NULL || provider->end == NULL) {
        return -1;
    }

    g_ggml_moe_expert_provider = *provider;
    return 0;
}
...
// Logical expert tensor storage used by MiniMaxBrain. The address range is
// reserved so GGML can assign stable tensor addresses without committing the
// full MoE weights to RAM. The CPU MUL_MAT_ID path must obtain actual expert
// bytes from the registered provider before dereferencing a routed weight.
struct ggml_moe_placeholder_ctx {
    void * base;
    size_t size;
};

static const char * ggml_moe_placeholder_name(ggml_backend_buffer_type_t buft) {
    GGML_UNUSED(buft);
    return "MMB_MOE_PLACEHOLDER";
}

static void ggml_moe_placeholder_free(ggml_backend_buffer_t buffer) {
    struct ggml_moe_placeholder_ctx * ctx = (struct ggml_moe_placeholder_ctx *) buffer->context;
    if (ctx != NULL) {
        if (ctx->base != NULL && ctx->size != 0) {
#if defined(_WIN32)
...
static void * ggml_moe_placeholder_get_base(ggml_backend_buffer_t buffer) {
    struct ggml_moe_placeholder_ctx * ctx = (struct ggml_moe_placeholder_ctx *) buffer->context;
    return ctx != NULL ? ctx->base : NULL;
}

static enum ggml_status ggml_moe_placeholder_init_tensor(
...
static void ggml_moe_placeholder_forbidden(
        ggml_backend_buffer_t buffer,
        struct ggml_tensor * tensor,
        uint8_t value,
        size_t offset,
    ...
    GGML_UNUSED(offset);
    GGML_UNUSED(size);
    GGML_ABORT("MMB placeholder expert storage cannot be accessed directly");
}

static void ggml_moe_placeholder_set(
        ggml_backend_buffer_t buffer,
        struct ggml_tensor * tensor,
        const void * data,
        size_t offset,
    ...
    GGML_UNUSED(offset);
    GGML_UNUSED(size);
    GGML_ABORT("MMB placeholder expert storage cannot be written directly");
}

static void ggml_moe_placeholder_get(
        ggml_backend_buffer_t buffer,
    ...
    GGML_UNUSED(offset);
    GGML_UNUSED(size);
    GGML_ABORT("MMB placeholder expert storage cannot be read directly");
}

static void ggml_moe_placeholder_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    GGML_UNUSED(buffer);
    GGML_UNUSED(value);
    // Nothing is committed. Clearing a logical weight buffer is a no-op.
}

static ggml_backend_buffer_t ggml_moe_placeholder_alloc(
        ggml_backend_buffer_type_t buft,
        size_t size) {
    struct ggml_moe_placeholder_ctx * ctx =
        (struct ggml_moe_placeholder_ctx *) calloc(1, sizeof(struct ggml_moe_placeholder_ctx));
    if (ctx == NULL) {
        return NULL;
    }
    ...
    if (size != 0) {
#if defined(_WIN32)
        ctx->base = VirtualAlloc(NULL, size, MEM_RESERVE, PAGE_NOACCESS);
        if (ctx->base == NULL) {
            free(ctx);
            return NULL;
        }
        ...
        flags |= MAP_NORESERVE;
#endif
        ctx->base = mmap(NULL, size, PROT_NONE, flags, -1, 0);
    ...
    const struct ggml_backend_buffer_i iface = {
        /* .free_buffer   = */ ggml_moe_placeholder_free,
        /* .get_base      = */ ggml_moe_placeholder_get_base,
        /* .init_tensor   = */ ggml_moe_placeholder_init_tensor,
        /* .memset_tensor = */ ggml_moe_placeholder_forbidden,
        /* .set_tensor    = */ ggml_moe_placeholder_set,
        /* .get_tensor    = */ ggml_moe_placeholder_get,
        /* .set_tensor_2d = */ NULL,
        /* .get_tensor_2d = */ NULL,
        /* .cpy_tensor    = */ NULL,
        /* .clear         = */ ggml_moe_placeholder_clear,
        /* .reset         = */ NULL,
    };
    return ggml_backend_buffer_init(buft, iface, ctx, size);
}

static size_t ggml_moe_placeholder_alignment(ggml_backend_buffer_type_t buft) {
    GGML_UNUSED(buft);
    return 64;
}

static bool ggml_moe_placeholder_is_host(ggml_backend_buffer_type_t buft) {
    GGML_UNUSED(buft);
    return false;
}

ggml_backend_buffer_type_t ggml_cpu_moe_placeholder_buffer_type(void) {
    static struct ggml_backend_buffer_type buft = {
        /* .iface = */ {
            /* .get_name       = */ ggml_moe_placeholder_name,
            /* .alloc_buffer   = */ ggml_moe_placeholder_alloc,
            /* .get_alignment  = */ ggml_moe_placeholder_alignment,
            /* .get_max_size   = */ NULL,
            /* .get_alloc_size = */ NULL,
            /* .is_host        = */ ggml_moe_placeholder_is_host,
        },
        /* .device  = */ NULL,
        /* .context = */ NULL,
    };

    // Keep device NULL like the standard CPU buffer type in this revision.
    // llama.cpp already treats a NULL buffer-type device as CPU.
    return &buft;
}
...
static void ggml_compute_forward_mul_mat_id(
    ...
    // Shared state for an optional external MoE expert provider.
    void ** moe_op_context =
        incr_ptr_aligned(&wdata_cur, sizeof(void *), sizeof(void *));
    const void ** moe_expert_data =
        incr_ptr_aligned(&wdata_cur, n_as * sizeof(void *), sizeof(void *));
    int32_t * moe_used_ids =
        incr_ptr_aligned(&wdata_cur, n_as * sizeof(int32_t), sizeof(int32_t));
    ...
    if (ith == 0) {
        ...
        *moe_op_context = NULL;
        memset(moe_expert_data, 0, n_as * sizeof(void *));

        if (g_ggml_moe_expert_provider.begin != NULL) {
            size_t moe_used_count = 0;
            for (int32_t expert = 0; expert < n_as; ++expert) {
                if (matrix_row_counts[expert] > 0) {
                    moe_used_ids[moe_used_count++] = expert;
                }
            }

            if (moe_used_count > 0) {
                const int rc = g_ggml_moe_expert_provider.begin(
                    g_ggml_moe_expert_provider.user_data,
                    src0->name,
                    moe_used_ids,
                    moe_used_count,
                    nb02,
                ...
                GGML_ASSERT(rc >= 0);
                if (rc == 0) {
                    GGML_ASSERT(*moe_op_context != NULL);

                    for (size_t i = 0; i < moe_used_count; ++i) {
                        const int32_t expert = moe_used_ids[i];
                        moe_expert_data[expert] = g_ggml_moe_expert_provider.get(
                            g_ggml_moe_expert_provider.user_data,
                            *moe_op_context,
                            expert,
                            nb02);
                        GGML_ASSERT(moe_expert_data[expert] != NULL);
                    }
                } else {
                    GGML_ASSERT(*moe_op_context == NULL);
                }
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        const char * src0_cur = moe_expert_data[cur_a] != NULL
            ? (const char *) moe_expert_data[cur_a]
    ...
    // No provider-backed pointer may be released until every worker has
    // finished the operation.
    ggml_barrier(params->threadpool);
    if (ith == 0 && *moe_op_context != NULL) {
        g_ggml_moe_expert_provider.end(
            g_ggml_moe_expert_provider.user_data,
            *moe_op_context,
            1);
        *moe_op_context = NULL;
                ...
                case GGML_OP_MUL_MAT_ID:
                    {
                        ...
                        // optional external MoE provider shared state
                        cur += sizeof(void *) + sizeof(void *);
                        cur += n_as * sizeof(void *) + sizeof(void *);
                        cur += n_as * sizeof(int32_t) + sizeof(int32_t);
                    } break;
                case GGML_OP_OUT_PROD:
                    {
                        if (ggml_is_quantized(node->src[0]->type) ||
