...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    if (files.empty()) {
        ...
        if (tid != -1) {
            ...
            // llama_model_init_from_user can be used with metadata that has no
            // tensor directory, in which case F32 remains the documented
            // default. When a tensor directory is present (as in MMB's
            // metadata-only GGUF), however, absence means the optional tensor
            // really is absent and must not be synthesized as a large F32
            // allocation.
            if (flags & TENSOR_NOT_REQUIRED) {
                return nullptr;
            }
            if (gguf_get_n_tensors(metadata) > 0) {
                throw std::runtime_error(format(
                    "missing required tensor '%s' in user-supplied GGUF metadata",
                    tn.str().c_str()));
            }
        }
        ...
        // Match ggml_new_tensor_impl stride construction, including block
        // quantized types (IQ*/Q*). The old virtual-loader path multiplied
        // ne[0] by the whole block byte size.
